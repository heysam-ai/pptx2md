...
from pptx2md.heysam_outputter import heysam_outputter
...
def parse_args():
    ...
    arg_parser.add_argument(
        "--disable-newline-removal-from-title",
        action="store_true",
        help="Disable the ability to remove newlines from title",
    )
    arg_parser.add_argument("--image-width", help="maximum image with in px", type=int)
    arg_parser.add_argument(
        "--disable-image", help="disable image extraction", action="store_true"
    )
    arg_parser.add_argument(
        "--disable-wmf",
    ...
    arg_parser.add_argument(
        "--disable-color", help="do not add color HTML tags", action="store_true"
    )
    arg_parser.add_argument(
        "--disable-escaping",
    ...
    arg_parser.add_argument(
        "--disable-notes", help="do not add presenter notes", action="store_true"
    )
    arg_parser.add_argument(
    ...
    arg_parser.add_argument(
        "--generic", help="generate output as generic markdown", action="store_true"
    )
    arg_parser.add_argument(
        "--min-block-size",
    ...
    arg_parser.add_argument(
        "--page", help="only convert the specified page", type=int, default=None
    )
    arg_parser.add_argument(
        "--has-too-many-links-ratio",
        help="if the percentage of chars in notes that are not part of a link are less than this threshold, filter out the notes",
        type=float,
        default=0.3,
    )
...
def main():
    ...
    g.has_too_many_links_ratio = args.has_too_many_links_ratio
    ...
    g.disable_newline_removal_from_title = (
        True if args.disable_newline_removal_from_title else False
    )
